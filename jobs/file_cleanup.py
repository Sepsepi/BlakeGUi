...
def get_last_processed_files():
    ...
    recent_files = get_recent_files('results', max_files=20)

    for file_info in recent_files:
        filename = file_info['name'].lower()

        # Enhanced categorization for phone files
        phone_keywords = [
            'phone_results', 'phone_extraction', 'phone_ready', 'zabasearch_ready',
            'phone', 'with_phones', 'without_phones', 'phone_search', 'phone_pipeline'
        ]

        # Enhanced categorization for address files
        address_keywords = [
            'address', 'bcpa', 'reverse', 'extracted', 'with_owners', 'owner_search',
            'property', 'bcpa_results', 'address_results'
        ]

        # Categorize files by type - phone takes priority over address for mixed files
        if any(keyword in filename for keyword in phone_keywords):
            results['phone'].append(file_info)
        elif any(keyword in filename for keyword in address_keywords):
