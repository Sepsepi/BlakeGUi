...
class BCPAOwnerSearch:
    ...
    def clean_address_for_search(self, row):
        """Clean and format address for BCPA search using enhanced parser results"""
        try:
            # First try to use the enhanced parser results if available
            if 'BCPA_Search_Format' in row and row['BCPA_Search_Format']:
                search_format = str(row['BCPA_Search_Format']).strip()
                if search_format and search_format.lower() not in ['nan', 'none', '']:
    ...
    async def process_csv(self, input_csv_path, output_csv_path=None):
        """Process CSV file to find missing owners using BCPA search with enhanced address parsing"""
        try:
            logger.info("=" * 60)
            logger.info("STARTING BCPA OWNER SEARCH WITH ENHANCED ADDRESS PARSING")
            logger.info("=" * 60)
...
def process_bcpa_lookup(csv_path: str, max_records: int = 50):
    ...
    try:
        ...
        # Run BCPA processing
        searcher = BCPAOwnerSearch(headless=True, delay_between_searches=1)
